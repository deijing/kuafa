...
def build_live_cover_prompt(
    ...
    *,
    ...
    """
    电商直播促销海报黄金提示词生成器（支持多参考图融合、16:9 横版 4K 与 9:16 竖版 2K 超清商业画质）：
    ...
    - 画质要求：竖版 9:16 采用 2K 超清分辨率商业广告级摄影画质（横版 16:9 采用 4K 超高清），细腻光影质感，极高清晰度与真实感。
    ...
    if multi_ref_count >= 2:
        ref_rule = (
            ...
            f"- 【融合呈现要求】：请将参考图中的主播人物与商品实物在{'横版 16:9 4K' if aspect_ratio == '16:9' else '竖版 9:16 2K'} 海报中完美融合！"
    ...
    return (
        f"{ref_rule}"
        f"混合媒介电商直播促销海报，竖版9:16比例，2K超清分辨率商业广告摄影画质，米白暖色纸张质感背景，整体采用{style_desc}，粗犷手绘刷痕边缘，具有强烈直播带货爆款视觉。\n\n"
        f"画面下半部为年轻亚洲主播的2K写实抠图展示，半身构图，手持展示「{prod_name}」，"
        f"柔和正面棚拍光，真实肤色，服装纹理超清细腻，自然阴影，人物边缘带白色描边与淡投影，人物五官端正清晰自然，严禁遮挡面部。\n\n"
        f"上半部为密集促销排版：\n"
        ...
        f"整体要求：2K 超清商业摄影画质，价格最抢眼，卖点明确，人物真实，信息层级清晰，热闹但不杂乱。\n\n"
        f"负面：错字乱码、文字缺失、标签裁切、额外人物或手机、品牌标志、肢体畸形、多余手指、面部变形、模糊、过曝、过饱和、杂乱背景、矢量化人物、塑料皮肤、低清晰度、遮挡面部、脸部被遮挡、居中大白框、手机UI、播放控件、底部小黄车按钮、视频字幕条。"
    )
...
def generate_video_covers(
    ...
    *,
    ...
    """
    基于成片真实画面与音频口播卖点，为成片自动化提取 3 帧高清画面并进行 AI 图生图（img2img）生成竖版 9:16 2K 高画质爆款封面海报。
    - 支持混剪期提前精选美学高光关键帧并同步并行生图。
    - 智能分析音频提炼多组爆款大字标语。
    - 每一帧作为图生图独立底图（竖版 9:16 构图、2K 超清），调用 AI 生成爆款海报。
    ...
    # 3. 如果配置了 AI 接口，使用真实截帧与音频智能提炼大字进行竖版 9:16 2K AI 图生图 (img2img)
