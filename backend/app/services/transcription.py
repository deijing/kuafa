...
_DANGLING_CONNECTIVES = re.compile(
    r"(因为|所以|而且|并且|不仅|但是|但|虽然|然后|如果|比如|比如讲|比如说|比方说|关于|这款|这个|那个|这些|那些|采用|含有|具有|主要|就是|而且它是|所以说|就是说|还有就是|另外就是|也就是说|其实|特别是|哪怕|即使|随着|由于|为了|只要|哪怕是|大家|想要|喜欢|需要|包含|包括|带一个|给你|给您|咱们|我们|你们|他们|它这个|它的|支持|非常|特别|十分|格外|极其|正在|准备|建议|推荐|能够|可以|一定要|无论是|不管是|哪怕是|不仅是|如果说|甚至|同时|另外|第一|第二|第三|首先|其次|最后|接下来|面料是|重量是|克重是|厚度是|尺码是|颜色是|颜色有|价格是|只要|立省|到手|直接去|赶紧去|抓紧去|抓紧|马上|你看这个|你看这款|我们家|这款衣服|说实话|坦白讲|不得不说)[，,、\s]*$"
)
